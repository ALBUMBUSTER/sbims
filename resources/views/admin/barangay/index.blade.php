...
<div class="main-container">
    ...
    <!-- ========== NEW: Children and Teenagers Statistics ========== -->
    <div class="info-box">
        <div class="info-box-header"><i class="fas fa-child"></i> Children & Teenagers</div>
        <ul class="info-list">
            @php
                $totalMinors = ($statistics['age_0_5'] ?? 0) + ($statistics['age_6_12'] ?? 0) + ($statistics['age_13_17'] ?? 0);
                $totalResidents = max($statistics['total_residents'] ?? 1, 1);
            @endphp
            <li class="info-list-item">
                <span class="info-list-label"><i class="fas fa-baby"></i> 0-5 years (Early Childhood)</span>
                <span class="info-list-value">{{ number_format($statistics['age_0_5'] ?? 0) }} ({{ round(($statistics['age_0_5'] ?? 0) / $totalResidents * 100) }}%)</span>
            </li>
            <li class="info-list-item">
                <span class="info-list-label"><i class="fas fa-chalkboard-user"></i> 6-12 years (Elementary)</span>
                <span class="info-list-value">{{ number_format($statistics['age_6_12'] ?? 0) }} ({{ round(($statistics['age_6_12'] ?? 0) / $totalResidents * 100) }}%)</span>
            </li>
            <li class="info-list-item">
                <span class="info-list-label"><i class="fas fa-school"></i> 13-17 years (High School)</span>
                <span class="info-list-value">{{ number_format($statistics['age_13_17'] ?? 0) }} ({{ round(($statistics['age_13_17'] ?? 0) / $totalResidents * 100) }}%)</span>
            </li>
            <li class="info-list-item" style="border-top: 1px dashed #cbd5e0; margin-top: 0.5rem; padding-top: 0.75rem;">
                <span class="info-list-label"><i class="fas fa-users"></i> Total Minors (0-17)</span>
                <span class="info-list-value">{{ number_format($totalMinors) }} ({{ round($totalMinors / $totalResidents * 100) }}%)</span>
            </li>
        </ul>
    </div>
    <!-- ========== END Children & Teenagers ========== -->

    <!-- ========== NEW: Adults and Seniors Statistics ========== -->
    <div class="info-box">
        <div class="info-box-header"><i class="fas fa-user"></i> Adults & Seniors</div>
        <ul class="info-list">
            @php
                $totalAdults = ($statistics['age_18_35'] ?? 0) + ($statistics['age_36_59'] ?? 0);
                $totalSeniors = $statistics['age_60_plus'] ?? 0;
            @endphp
            <li class="info-list-item">
                <span class="info-list-label"><i class="fas fa-briefcase"></i> 18-35 years (Young Adults)</span>
                <span class="info-list-value">{{ number_format($statistics['age_18_35'] ?? 0) }} ({{ round(($statistics['age_18_35'] ?? 0) / $totalResidents * 100) }}%)</span>
            </li>
            <li class="info-list-item">
                <span class="info-list-label"><i class="fas fa-user-tie"></i> 36-59 years (Middle Age)</span>
                <span class="info-list-value">{{ number_format($statistics['age_36_59'] ?? 0) }} ({{ round(($statistics['age_36_59'] ?? 0) / $totalResidents * 100) }}%)</span>
            </li>
            <li class="info-list-item">
                <span class="info-list-label"><i class="fas fa-cane"></i> 60+ years (Senior Citizens)</span>
                <span class="info-list-value">{{ number_format($statistics['age_60_plus'] ?? 0) }} ({{ round(($statistics['age_60_plus'] ?? 0) / $totalResidents * 100) }}%)</span>
            </li>
            <li class="info-list-item" style="border-top: 1px dashed #cbd5e0; margin-top: 0.5rem; padding-top: 0.75rem;">
                <span class="info-list-label"><i class="fas fa-users"></i> Total Adults (18-59)</span>
                <span class="info-list-value">{{ number_format($totalAdults) }} ({{ round($totalAdults / $totalResidents * 100) }}%)</span>
            </li>
            <li class="info-list-item">
                <span class="info-list-label"><i class="fas fa-user-plus"></i> Total Seniors (60+)</span>
                <span class="info-list-value">{{ number_format($totalSeniors) }} ({{ round($totalSeniors / $totalResidents * 100) }}%)</span>
            </li>
        </ul>
    </div>
    <!-- ========== END Adults & Seniors ========== -->
    ...
    <ul class="info-list">
        <li class="info-list-item">
            ...
            <span class="info-list-value">
                @if($barangayInfo->barangay_captain)
                    {{ $barangayInfo->barangay_captain }}
                @elseif(isset($captain) && $captain)
                    {{ $captain->full_name }}
                @else
        ...
        <li class="info-list-item">
            ...
            <span class="info-list-value">
                @if($barangayInfo->barangay_secretary)
                    {{ $barangayInfo->barangay_secretary }}
                @elseif(isset($secretary) && $secretary)
                    {{ $secretary->full_name }}
                @else
                    <span class="text-muted">Not set</span>
                @endif
        ...
        <div class="barangay-detail-section">
            ...
            <div class="chart-tabs">
                ...
                <button class="chart-tab" onclick="switchChartView(event, 'age')">
...
<div class="chart-card" data-chart="age">
    <div class="chart-header">
        <h4><i class="fas fa-chart-line"></i> Age Distribution</h4>
        <span class="stat-highlight">{{ $statistics['total_residents'] ?? 0 }} total residents</span>
    </div>
    <div class="chart-container">
        ...
        <form action="{{ route('admin.barangay.update-officials') }}" method="POST">
            ...
            <div class="stat-input-group">
                ...
                <input type="text" id="barangay_captain" name="barangay_captain"
                    value="{{ old('barangay_captain', $barangayInfo->barangay_captain ?? ($captain->full_name ?? '')) }}"
                    placeholder="Enter barangay captain's name">
                <small style="color: #666; font-size: 0.75rem;">This will also update the captain's user account</small>
            </div>
            ...
            <div class="stat-input-group">
                ...
                <input type="text" id="barangay_secretary" name="barangay_secretary"
                    value="{{ old('barangay_secretary', $barangayInfo->barangay_secretary ?? ($secretary->full_name ?? '')) }}"
                    placeholder="Enter barangay secretary's name">
                <small style="color: #666; font-size: 0.75rem;">This will also update the secretary's user account</small>
            </div>
...
<script>
    ...
    function getStatisticsData() {
        ...
        return {
            ...
            // ========== AGE DISTRIBUTION ==========
            ageGroups: {
                '0-5 years': getStat('age_0_5'),
                '6-12 years': getStat('age_6_12'),
                '13-17 years': getStat('age_13_17'),
                '18-35 years': getStat('age_18_35'),
                '36-59 years': getStat('age_36_59'),
                '60+ years': getStat('age_60_plus')
            }
        };
    }
...
function createAgeChart(stats) {
    const ctx = document.getElementById('ageChart')?.getContext('2d');
    if (!ctx) return;

    const ageLabels = Object.keys(stats.ageGroups);
    const ageData = Object.values(stats.ageGroups);

    // Colors matching the certificate chart style
    const ageColors = [
        '#10b981', // 0-5 years - Green
        '#3b82f6', // 6-12 years - Blue
        '#8b5cf6', // 13-17 years - Purple
        '#f59e0b', // 18-35 years - Orange
        '#ef4444', // 36-59 years - Red
        '#6b7280'  // 60+ years - Gray
    ];
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            ...
                            const total = stats.total;
                            const percentage = total > 0 ? ((value / total) * 100).toFixed(1) : 0;
                            return `${value.toLocaleString()} residents (${percentage}%)`;
                        }
                    }
                }
            },
    ...
    // Create custom legend matching certificate chart style
    const legendContainer = document.getElementById('ageLegend');
    if (legendContainer) {
        legendContainer.innerHTML = '';
        ageLabels.forEach((label, index) => {
            const total = stats.total;
            const count = ageData[index];
            const percentage = total > 0 ? ((count / total) * 100).toFixed(1) : 0;

            const legendItem = document.createElement('div');
            legendItem.className = 'legend-item';
            legendItem.innerHTML = `
                <div class="legend-color" style="background: ${ageColors[index]};"></div>
                <span>${label}: ${count.toLocaleString()} (${percentage}%)</span>
            `;
